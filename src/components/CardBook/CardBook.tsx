...
const CardBook = ({ book }: { book: IBook }) => {
  ...
  return (
    <div
      className="relative w-28 mb:w-36 mb-2 flex flex-col items-center justify-center rounded-xl shadow-sm border border-gray-100 cursor-pointer"
      ...
      {book.status === "lido" ? (
        <>
          <div className="absolute top-0 right-2 text-gray-300 cursor-pointer">
            <FaHeart
              className={`text-2xl mb:text-3xl ${
                favorited ? "text-red-600" : "text-gray-400"
              }`}
              onClick={(e) => favoriteBook(e)}
            />
          </div>
          <div className="w-24 h-36 mb:w-28 mb:h-42 mt-3 shrink-0">
            <img
          ...
          <div className="w-24 mb:w-28 flex mb:my-1">
            {[1, 2, 3, 4, 5].map((star) => (
        ...
        <div className="w-24 h-36 mb:w-28 mb:h-42 my-2 shrink-0 flex flex-col justify-center">
          <img
